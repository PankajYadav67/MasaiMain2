...
export const PinInputComponents = (props: PinInputProps) => {
    ...
    const handleChange = (e: ChangeEvent<HTMLInputElement>, index: number) => {
        ...
        setIsAllFilled(pinArray.every((value) => value.length >= limitPerBox));
    }
    ...
    return (
        <div className={`${styles.pinInputWrapper} ${isAllFilled ? styles.allFilled : ""}`} onPaste={(e) => {
            e.preventDefault();
            ...
            {pinArray.map((val, index) => (
                <input
                    key={index}
                    className={`${styles.pinInputFieldWrapper} ${isAllFilled ? styles.allFilled : ""}`}
